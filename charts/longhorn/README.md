...
| defaultSettings.systemManagedComponentsPriorityClasses | Per-component PriorityClass overrides for system-managed Longhorn components. Unset components fall back to `defaultSettings.priorityClass`. Supported components: instance-manager, engine-image, longhorn-csi-plugin, csi-attacher, csi-provisioner, csi-resizer, csi-snapshotter. WARNING: DO NOT CHANGE THIS SETTING WITH ATTACHED VOLUMES. The value should be a JSON object with component names as keys and PriorityClass names as values. |
